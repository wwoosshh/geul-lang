...
- 레지스터 할당 (D-20): 정수·참조 값을 RBX·RSI·RDI·R12–R15 에 두는 선형 스캔 할당과 레지스터 중심 내보내기(결과 레지스터 계산, 별칭, 주소 접기, 비교·분기 이음). IR 은 그대로이고 두 구현이 같은 바이트를 낸다. 반복문 벤치마크 0.45초→0.26초, 컴파일러 exe 667KB→468KB (자기 컴파일 시간 0.15초는 입출력이 대부분이라 그대로).
