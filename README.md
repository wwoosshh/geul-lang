...
2026-09-08부터 이틀간 시도한 검토 언어 방향은 중단하고 1.1로 되돌렸다. 이유와 탈락한 여덟 갈래는
[연구 중단 기록](연구-중단-기록-2026-09-09.md)에 있다. 당시 산출물은 가지 `research/review-language`에 보존한다.
...
**1.1.** 문법과 표준 라이브러리는 [호환 약속](docs/11-호환-약속.md)으로 얼려 있다.
1.1 에서 더한 것은 표준 함수 `파일_수정시각`, 외부 선언의 DLL 이름(D-38), 컴파일러 옵션 `--창`(D-39)이다.
전부 더하는 변화이므로 1.0 코드는 그대로 돈다.

| | |
|---|---|
| 참조 구현 (파이썬) | 6,787줄 / 19파일 |
| 자체 컴파일러 (글로 쓴 글 컴파일러) | 9,259줄 / 13파일 |
| 표준 라이브러리 | 2,771줄 / 14모듈 |
| 테스트 프로그램 | 4,715줄 / 190개 |
| 글로 쓴 도구 | 341줄 / 3파일 |
| 문서 | 2,637줄 / 12편 |
| 설계 결정 기록 | D-01 ~ D-39 |
...
| 검증 | 결과 |
|---|---|
| 수용 테스트 (참조 구현 / 글 컴파일러) | 190/190 양쪽 |
| 덤프 교차 비교 (토큰·구문·IR) | 158개 입력 전부 일치 |
| 호출 색인 · 위험 보고 교차 비교 | 각 158개 일치 |
| 오류 메시지 (부정 테스트) | 59개 전부 같은 줄·열·문구 |
| 실행 파일 바이트 비교 | 135개 전부 동일 |
| 자기 컴파일 고정점 | 1·2·3세대 바이트 동일 |
| 문서 예제 | 27/27 |
| 글로 쓴 도구 | 2/2 |

글로 쓴 컴파일러가 자기 자신을 컴파일해 참조 구현과 **바이트가 같은** 실행 파일을 낸다.
만들어진 실행 파일은 C 런타임 없이 `kernel32.dll` 과 `shell32.dll` 만 쓴다 (창 프로그램은
`user32`·`gdi32`·`shell32` 를 더 쓴다).
