...
표준 라이브러리 11개 모듈(기본·서식·문자열·입출력·수학·목록·배열·사전·바이트열·시간·체계)은 C 런타임 없이 kernel32 위에 글로만 쓰여 있고, 글로 쓴 컴파일러(`self/`)가 자기 자신을 컴파일해 바이트 동일한 실행 파일을 냅니다(고정점). 참조 구현과 토큰·AST·IR 덤프, exe 바이트가 모두 일치하고, 글 컴파일러로도 수용 테스트를 전부 통과합니다 (`python build.py selfhost`, `python build.py test --self`).
